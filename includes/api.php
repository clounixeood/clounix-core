...
if (ini_get('register_globals') != 1) { 
    ...
    foreach($supers as $__s) if (isset($$__s)  && is_array($$__s)) { extract($$__s, EXTR_OVERWRITE); }
    ...
    function pnInit()
    {
        ...
        // Connect to database
        if (!pnDBInit()) die('Inizializzazione database fallito');
        ...
        foreach (glob("includes/ClassDefault/*.php") as $filename) { include $filename; }

         
        // Start session
        if (!pnSessionSetup())  die('Setup sessione fallito');
        if (!pnSessionInit())   die('Inizializzazione sessione fallita');
    ...
    function pnVarCleanFromPost($array, $keycheck = 1)
    {
        ...
        if (($keycheck == 1) && ($array['form_hashkey'] != PnSessionGetVar('form_hashkey'))) { PnRedirect(PnModUrl('Users', 'user', 'FormError')); }
        ...
        foreach ($array as $key => $var) {
            ...
            if (!get_magic_quotes_gpc()) { $var = htmlentities(addslashes($var), ENT_COMPAT, "UTF-8"); }
    ...
    function pnStripslashes(&$value)
    {
        ...
        else array_walk($value, 'pnStripslashes');
    ...
    function pnGetBaseURI()
    {
        ...
        if (empty($path)) { $path = pnServerGetVar('SCRIPT_NAME'); }
    ...
    function pnGetBaseHost()
    {
      ...
      if (isset($_SERVER['HTTPS'])) { $proto = 'https://'; } else { $proto = 'http://'; }
    ...
    function pnLocalReferer()
    {
      ...
      if (empty($referer) || preg_match("!^http://$server/!", $referer)) { return true; } else { return false; }
    }
    ...
    function pnGetHost()
    {
        ...
        if (empty($server)) { $server = pnServerGetVar('SERVER_NAME'); $port = pnServerGetVar('SERVER_PORT'); if ($port != '80') $server .= ":$port"; }
    ...
    function VarHtmlDecode()
    {
        ...
        // Return vars
        if (func_num_args() == 1) return $resarray[0]; else return $resarray;
    } 
    ...
    function CheckAdminAuth() {
        ...
        $gruppo = PnUserGetMemberOf($userid);
    
        //Se utente non è loggato
        if (!PnUserLoggedIn()) { Pnredirect(pnModUrl('Users', 'user', 'NoAuth')); exit; return false; }
        ...
        //Se non è admin esco
        if ($admin[0] != 1) { Pnredirect(pnModUrl('Users', 'user', 'NoAuth'));  exit; return false; } 
        ...
        if (in_array($func, $linkdb))       {  $permesso = 1;  }
        if (in_array("*", $linkdb))         {  $permesso = 1;  }
        if (in_array("!$func", $linkdb))    {  $permesso = 0;  }
        ...
        if ($permesso == 1) { return true; 
            
        //Se non ha il permesso
        } else { 
        
            if ((!$refer) || stristr($refer, $link)) { $refer = PnModUrl('Administration', 'admin', 'Main');  }
    ...
    function CutString($string, $char) {
        ...
        if (strlen($string) > $char) { $string = ''.substr($string, 0, $char).'..'; } else { $string = $string; }
  ...
  return true; 
    ...
    function PutImage($attribute = array(), $func = __FUNCTION__) {
        ...
        foreach ($attribute as $key => $value)  { $buf .= ''.(($key == 'singletag') ? ' '.$value.'' : ' '.$key.'="'.$value.'"').''; }
